...
- **Data collection (manifest):** `data_collection_permissions.required = ["none"]` — declare "No" data collection in the AMO form; it is consistent with the manifest and PRIVACY.md.

### Summary (≤250 chars)

Microsoft Entra ID single sign-on for Firefox on Linux. Bridges to the local
microsoft-identity-broker so Firefox can satisfy Conditional Access policies that
otherwise only work in Microsoft Edge.

### Full description

On Linux, Microsoft Entra ID Conditional Access (Intune device compliance) normally
works only in Microsoft Edge, because only Edge integrates with the Microsoft
Identity Broker. Linux Entra Bridge closes that gap for Firefox.

A small native messaging host (Python) talks to the local
`microsoft-identity-broker` D-Bus service, obtains the PRT SSO cookie, and the
extension places it in the browser cookie store for the Microsoft sign-in domains.
Firefox then presents as a compliant device to Entra ID — for Microsoft 365, Azure
Portal, and any SAML/OAuth application behind `login.microsoftonline.com`.

Requirements: a Linux device enrolled in Microsoft Intune with
`microsoft-identity-broker` running, plus the native messaging host from the project
page. Everything runs locally; the extension makes no network requests of its own.
